...
Три готовые модели, выложенные открыто и обученные другими. На занятии не обучается
ни одна, все только применяются. Этих кадров ни одна из них при обучении не видела.

| Модель | Что даёт | Для чего обычно нужна |
| --- | --- | --- |
| Запрос обычными словами | рамки, контуры и номера объектов за один проход. 860 млн параметров, 3,3 ГБ, от 0,43 до 0,63 с на кадр | поиск объектов, которых не было в обучающей выборке |
| Глубина по снимку | относительную карту расстояний по одной фотографии, 95 МБ | оценка расстояний там, где дальномер не поставить |
| Описание участка числами | набор чисел на каждый участок изображения, 327 МБ | поиск похожего, подготовка выборок, сверка с образцом |

Веса лежат на машине заранее, в сеть за ними никто не ходит.

Первая модель закрывает три задачи сразу: обнаружение, выделение контура и сохранение
номера во времени. Поэтому смена одного слова в запросе меняет и рамки, и контуры,
и подсчёт объектов.
"""),
