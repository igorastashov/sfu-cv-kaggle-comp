...
| --- | --- | --- |
| SAM 3, запрос обычными словами | рамки, контуры и номера объектов за один проход. 860 млн параметров, 3,3 ГБ, от 0,43 до 0,63 с на кадр | поиск объектов, которых не было в обучающей выборке |
| Depth Anything V2 Small | относительную карту расстояний по одной фотографии, 95 МБ | оценка расстояний там, где дальномер не поставить |
| DINOv3 ViT-B/16 | набор чисел на каждый участок изображения, 327 МБ | поиск похожего, подготовка выборок, сверка с образцом |

Веса лежат на машине заранее, в сеть за ними никто не ходит.

SAM 3 закрывает три задачи сразу: обнаружение, выделение контура и сохранение
